...
export default function FacilitiesPage() {
  const [hospitals, setHospitals] = useState<Hospital[]>(ALL_SP_HOSPITALS);
  ...
  const loadFacilities = async () => {
    try {
      const data = await api.get<{ facilities: Hospital[] }>('/facilities');
      if (data && data.facilities && data.facilities.length > 0) {
        // Enrich local reference catalog with backend CNES info preserving all zone attributes
        const merged = ALL_SP_HOSPITALS.map(localH => {
          const remoteH = data.facilities.find(rf => rf.id === localH.id || rf.name.toLowerCase().includes(localH.name.toLowerCase().slice(0, 10)));
          return {
            ...localH,
            ...(remoteH || {}),
            zone: localH.zone,
            district: localH.district,
            network: localH.network,
            is_public: localH.is_public,
            is_24h: localH.is_24h,
            is_emergency: localH.is_emergency
          };
        });
        setHospitals(merged);
      }
    } catch (err) {
      console.warn('Utilizando catálogo local de hospitais:', err);
    }
  };

  // Helper count for each zone
  const getZoneCount = (z: string) => {
    if (z === 'Todas') return hospitals.length;
    return hospitals.filter(h => (h?.zone || '').trim().toLowerCase() === z.trim().toLowerCase()).length;
  };

  // Helper count for each type
  const getTypeCount = (t: string) => {
    if (t === 'Todos') return hospitals.length;
    if (t === '24h') return hospitals.filter(h => Boolean(h?.is_24h)).length;
    if (t === 'Emergência') return hospitals.filter(h => Boolean(h?.is_emergency)).length;
    if (t === 'SUS') return hospitals.filter(h => Boolean(h?.is_public || h?.network === 'SUS' || h?.network === 'Filantrópico')).length;
    if (t === 'Privado') return hospitals.filter(h => Boolean(!h?.is_public || h?.network === 'Privado')).length;
    return 0;
  };
  ...
  const filteredHospitals = useMemo(() => {
    return hospitals.filter(h => {
      ...
      if (filterType === '24h') matchesType = Boolean(h?.is_24h);
      else if (filterType === 'Emergência') matchesType = Boolean(h?.is_emergency);
      else if (filterType === 'SUS') matchesType = Boolean(h?.is_public || h?.network === 'SUS' || h?.network === 'Filantrópico');
      else if (filterType === 'Privado') matchesType = Boolean(!h?.is_public || h?.network === 'Privado');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      <div className="hud-card" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {/* Zone Filter with Live Counters and Active Indicators */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
            ...
            {['Todos', '24h', 'Emergência', 'SUS', 'Privado'].map(t => {
